...
namespace dbgl
{
    ...
    WindowGL33::WindowGL33(std::string title, int width, int height, bool fullscreen,
	    unsigned int multisampling) : m_title { title }, m_multisampling { multisampling }
    {
	if(fullscreen)
	{
	...
	if(fullscreen)
	    m_pWndHandle = glfwCreateWindow(m_fullscreenWidth, m_fullscreenHeight, title.c_str(), glfwGetPrimaryMonitor(), nullptr);
	else
    ...
    WindowGL33::IconifiedEventType::DelegatePtr WindowGL33::addIconifiedCallback(
    ...
    {
	s_windows[window]->m_iconifiedCallbacks.fire(WindowGL33::IconifiedEventArgs{iconified == GL_TRUE ? true : false});
    }
    ...
    {
	s_windows[window]->m_cursorEnterCallbacks.fire(WindowGL33::CursorEnterEventArgs{entered == GL_TRUE ? true : false});
    }
	...
	s_windows[window]->m_mouseButtonCallbacks.fire(WindowGL33::MouseButtonEventArgs{keyConst, keyState, s_windows[window]->m_input});
	s_windows[window]->m_inputCallbacks.fire(WindowGL33::InputEventArgs{keyConst, keyState, s_windows[window]->m_input});
    }
