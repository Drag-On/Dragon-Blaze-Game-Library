...
namespace dbgl
{
	...
	Log::Logger::LogBuf::LogBuf(Level loglevel)
			: m_loglevel(loglevel)
	{
	}
	...
	int Log::Logger::LogBuf::sync()
	{
		...
		{
		case Level::DBG:
			Log::getDefault().debug(str().c_str());
			break;
		case Level::INFO:
			Log::getDefault().info(str().c_str());
			break;
		case Level::WARN:
			Log::getDefault().warning(str().c_str());
			break;
		case Level::ERR:
			Log::getDefault().error(str().c_str());
			break;
		}
		str("");
		return 0;
	...
	Log::Logger::Logger(Level loglevel)
			: std::ostream(new LogBuf(loglevel))
	{
	}
