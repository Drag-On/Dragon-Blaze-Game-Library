...
namespace dbgl
{
	...
	public:
		...
		template<class RandomAccessIterator1, class RandomAccessIterator2> KdTree(RandomAccessIterator1 beginPoints,
				RandomAccessIterator1 endPoints, RandomAccessIterator2 beginDat, RandomAccessIterator2 endDat);
		/**
	...
	private:
		 ...
		 */
		template<class RandomAccessIterator> Node* buildTree(RandomAccessIterator begin, RandomAccessIterator end,
				unsigned int curDepth = 0, Node* parent = nullptr) const;
		 ...
		 * @return Pointer to the node with the passed coordinates or NULL if not found
		 */
		Node* searchForSimilar(Point const& point, Node* node, unsigned int curDepth, unsigned int *level = nullptr,
				double precision = 0.001) const;
		 ...
		 */
		void findNearestNeighbor(Point const& point, Node const& node, NearestNeighbor& currentBest, bool goDown,
