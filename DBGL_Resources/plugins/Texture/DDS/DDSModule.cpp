...
namespace dbgl
{
	...
	public:
		...
		fileHeader.pitchLinearSize = (fHeader[20] << 0) | (fHeader[21] << 8) | (fHeader[22] << 16) | (fHeader[23] << 24);
		fileHeader.depth = (fHeader[24] << 0) | (fHeader[25] << 8) | (fHeader[26] << 16) | (fHeader[27] << 24);
		fileHeader.mipMapCount = (fHeader[28] << 0) | (fHeader[29] << 8) | (fHeader[30] << 16) | (fHeader[31] << 24);
		/* fileHeader.reserved */
		fileHeader.pixelFormat.size = (fHeader[76] << 0) | (fHeader[77] << 8) | (fHeader[78] << 16) | (fHeader[79] << 24);
		fileHeader.pixelFormat.flags = (fHeader[80] << 0) | (fHeader[81] << 8) | (fHeader[82] << 16) | (fHeader[83] << 24);
		fileHeader.pixelFormat.fourCC = (fHeader[84] << 0) | (fHeader[85] << 8) | (fHeader[86] << 16) | (fHeader[87] << 24);
		fileHeader.pixelFormat.rgbBitCount = (fHeader[88] << 0) | (fHeader[89] << 8) | (fHeader[90] << 16) | (fHeader[91] << 24);
		fileHeader.pixelFormat.rBitMask = (fHeader[92] << 0) | (fHeader[93] << 8) | (fHeader[94] << 16) | (fHeader[95] << 24);
		fileHeader.pixelFormat.gBitMask = (fHeader[96] << 0) | (fHeader[97] << 8) | (fHeader[98] << 16) | (fHeader[99] << 24);
		fileHeader.pixelFormat.bBitMask = (fHeader[100] << 0) | (fHeader[101] << 8) | (fHeader[102] << 16) | (fHeader[103] << 24);
		fileHeader.pixelFormat.aBitMask = (fHeader[104] << 0) | (fHeader[105] << 8) | (fHeader[106] << 16) | (fHeader[107] << 24);
		...
		// Analyze data
		ITexture::PixelFormatCompressed format;
		switch (fileHeader.pixelFormat.fourCC)
		{
		    case static_cast<uint32_t>(FourCC::DXT1):
			format = ITexture::PixelFormatCompressed::COMP_DXT1;
			break;
		    case static_cast<uint32_t>(FourCC::DXT3):
			format = ITexture::PixelFormatCompressed::COMP_DXT3;
			break;
		    case static_cast<uint32_t>(FourCC::DXT5):
			format = ITexture::PixelFormatCompressed::COMP_DXT5;
			break;
		    default:
		...
		unsigned int blockSize = (format == ITexture::PixelFormatCompressed::COMP_DXT1) ? 8 : 16;
		unsigned int width = fileHeader.width;
		unsigned int height = fileHeader.height;
		// Load mipmaps
		for (unsigned int level = 0;
			level < fileHeader.mipMapCount && (fileHeader.width || fileHeader.height); ++level)
		{
