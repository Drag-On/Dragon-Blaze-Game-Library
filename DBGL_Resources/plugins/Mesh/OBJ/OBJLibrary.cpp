...
namespace dbgl
{
	...
	private:
	    ...
	    template <typename T> T to(std::string const& val) const
	    {
	     ...
	     */
	    template <typename T> void extract(std::string const& line, std::string& first, std::vector<T>& container) const
	    {
		std::istringstream lineStream{};
		lineStream.str(line);
		lineStream >> first;
